...
{{ old('description', $occasion->description) }}
    ...
    <form action="{{ route('occasion.destroy', $occasion->id) }}" method="POST">
        <div class="flex sm:justify-end justify-center mt-4">
            ...
            @method('DELETE')
            <button type="submit"
                class="rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-red-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-600">Verwijder
                occasion</button>
        </div>
    </form>
